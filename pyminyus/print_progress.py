...
def format_sec(s):
    ...
    h = m/60
    d = h/24
    if d >= 1: return '{:5.2f} days'.format(d)
    if h >= 1: return '{:5.2f} hrs '.format(h)
    if m >= 1: return '{:5.2f} mins'.format(m)
...
class PrintProgress():
    def __init__(self, total_iterations, overwrite=False, update_interval_sec=1.0, prefix=''):
    ...
    def print_progress(self, i):
        ...
        if ((t - self._time_updated) > self.update_interval_sec) or (i in [0, self.last_iteration]):
            ...
            print_str = '[' + timestamp() + '] ' + ' | '.join([self.prefix,
                 '{:d}/{:d}={:5.2f}%'.format(i_, self.total_iterations, progress_pct),
